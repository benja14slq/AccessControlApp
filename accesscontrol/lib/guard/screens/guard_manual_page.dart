...
class _GuardManualPageState extends State<GuardManualPage> {
  ...
  List<Map<String, dynamic>> _getFilteredResidents() {
    ...
    return widget.state.residentsList
        .where((r) => r['torre'] == _selectedTower)
        .toList();
  }
  ...
  Future<void> _processAction({required bool isNotification}) async {
    ...
    if (isNotification && _visitorNameCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Ingresa nombre del visitante')),
      );
      return;
    }
    ...
    final residentData = widget.state.residentsList.firstWhere(
      (r) => r['uid'] == _selectedResidentUid,
    );
  ...
  void _onNotificationUpdate(DocumentSnapshot snapshot) {
    ...
    if (status == 'aprobada') {
      widget.state.logApprovedVisit(
        visitorName: data['visitorName'] ?? 'Visitante',
        residentName: data['residentName'] ?? 'Residente',
  ...
  void _updateUI(Map<String, dynamic> result) {
    if (mounted) {
      setState(() {
        ...
        _isSuccess =
            result['status'] == 'ok' || result['status'] == 'notificando';
        _isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    final unannouncedEnabled =
        widget.state.condoConfig['unannouncedVisitsEnabled'] ?? true;
        ...
        child: Column(
          ...
          children: [
            ...
            if (hasTowers)
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(
                  labelText: 'Seleccionar Torre',
                  border: OutlineInputBorder(),
                ),
                items: widget.state.towers
                    .map(
                      (t) =>
                          DropdownMenuItem(value: t, child: Text('Torre $t')),
                    )
                    .toList(),
                onChanged: (val) {
                  setState(() {
            ...
            DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(
                labelText: 'Seleccionar Unidad/Residente',
                border: OutlineInputBorder(),
              ),
            ...
            if (unannouncedEnabled) ...[
              ...
              const SizedBox(height: 16),
              FilledButton.icon(
                onPressed: (_isLoading || _isNotifying)
                    ? null
                    : () => _processAction(isNotification: true),
                icon: _isNotifying
                    ? const SizedBox(
                        width: 20,
                        height: 20,
                ...
                label: Text(
                  _isNotifying ? 'Esperando respuesta...' : 'Notificar Visita',
                ),
            ...
            OutlinedButton.icon(
              onPressed: (_isLoading || _isNotifying)
                  ? null
                  : () => _processAction(isNotification: false),
